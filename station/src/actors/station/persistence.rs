...
pub fn build_rent_json(rental_id: &str, user_id: UserId, bike_id: BikeId, card_token: &str) -> String {
...
pub fn save_pending_rent(station_id: StationId, rental_id: &str, user_id: UserId, bike_id: BikeId, card_token: &str) {
...
pub fn save_pending_charge(station_id: StationId, rental_id: &str, amount_cents: u32, bike_id: BikeId) {
...
fn append_to_file(filename: &str, content: &str) {
    std::fs::OpenOptions::new()
        ...
        .unwrap_or_else(|e| eprintln!("[ERROR PERSISTENCIA] No se pudo escribir en {}: {}", filename, e));
}
...
pub fn flush_pending_file(filename: &str, lines_to_keep: &[String]) {
    if lines_to_keep.is_empty() {
        ...
        std::fs::write(filename, lines_to_keep.join("\n") + "\n")
            .unwrap_or_else(|e| eprintln!("[ERROR PERSISTENCIA] No se pudo actualizar {}: {}", filename, e));
    }
}
